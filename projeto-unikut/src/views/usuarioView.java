...
import FactoryMethod.RecadoDiversos;
import FactoryMethod.RecadoInterface;
import FactoryMethod.RecadoJogos;
import FactoryMethod.RecadoSaudação;
import FactoryMethod.RecadoSeries;
import static controllers.controllerRecados.buscaSenhaSecreta;
import static controllers.controllerRecados.buscaUsuarioRecado;
...
public class usuarioView extends Usuario {
    ...
    public void exibeConversa(List<Usuario> usuarios) {
        ...
        Usuario busca = controllerUsuario.buscaUsuarioLogin(login, usuarios);
        if (busca == null) {
            System.err.println("Login inexistente!");
        } else {
            Runnable t1 = () -> System.out.println(busca.getRecados());
            new Thread(t1).start();
        }
    }
    ...
    public void alterarDados(List<Usuario> usuarios) {
        ...
        String login = in.nextLine();
        Usuario buscaLogin;
        buscaLogin = buscaUsuarioLogin(login, usuarios);
        if (buscaLogin != null) {
        ...
        if (retornaUsuario(meuLogin, usuarios) == null) {
            ...
            if (conta != null) {
                if (conta == retornaUsuario(meuLogin, usuarios)) {
                    System.err.println("Você não pode adicionar a si mesmo!");
                } else {
    ...
    protected void enviaRecados(List<Usuario> usuarios) {
        ...
        switch (op) {
            case 1:
                RecadoInterface mensagemJogos = new RecadoJogos();
                mensagemJogos.enviaRecados(usuarios);
                break;
            case 2:
                RecadoInterface mensagemSaudaçao = new RecadoSaudação();
                mensagemSaudaçao.enviaRecados(usuarios);
                break;
            case 3:
                RecadoInterface mensagemDiversos = new RecadoDiversos();
                mensagemDiversos.enviaRecados(usuarios);
                break;
            case 4:
                RecadoInterface mensagemSeries = new RecadoSeries();
                mensagemSeries.enviaRecados(usuarios);
                break;
            case 0:
        ...
        if (retornaUsuario(meuLogin, usuarios) == null) {
            ...
            if (conta == null) {
                ...
                if (!matchPendentes.contains(retornaUsuario(meuLogin, usuarios)) && !matchh.contains(conta)) {
                    ...
                    System.out.println(ANSI_GREEN + "MATCH ENVIADO!" + ANSI_RESET);
                } else {
                    ...
                    System.out.println(ANSI_GREEN + ANSI_WHITE_BACKGROUND + "AMBOS DERAM MATCH!" + ANSI_RESET);
                }
            }
        }
    ...
    protected void aceitaMensagemMural(Usuario usuario, List<Usuario> usuarios, Usuario mural) {
        ...
        if (retornaUsuario(meuLogin, usuarios) == null) {
            ...
            System.out.print(ANSI_WHITE + "->" + ANSI_RESET);
            String login = in.nextLine();
            Recado conta = buscaUsuarioRecado(login, mensagensP);
            if (conta == null) {
                System.err.println("LOGIN INVÁLIDO!");
            } else {
    ...
    protected void exibeRecadosSecretos(List<RecadoSecreto> recadosSec, List<Usuario> usuarios) {
        ...
        if (retornaUsuario(meuLogin, usuarios) == null) {
            ...
            RecadoSecreto recadoSecreto = buscaSenhaSecreta(senhaRecSecreto, recadosSec);
    ...
    public static Usuario retornaUsuario(String login, List<Usuario> usuarios) {
        Usuario buscaLogin = buscaUsuarioLogin(login, usuarios);
        return buscaLogin;
    }
}
